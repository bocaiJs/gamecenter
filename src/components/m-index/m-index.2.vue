<template>
    <div class="gameindex" ref="gameindex">
        <scroll class="index-wrapper" :listenScroll=true :data=data>
            <div>
                <div style="position:relative;overflow:hidden;width:100%" v-if="adsList.length">
                    <slider :sliderData="sliderData" ref="sliderDom">
                        <div v-for="(item,index) in adsList" :key=index>
                            <div class="item" @click.stop.prevent="goGame(item.url)">
                ...
                <div class="index-tab">
                    <router-link tag="div" class="index-tab-item" to="/index/libao">
                        <span class="tab-icon libao"></span>
                        <p class="tab-link">领取礼包</p>
                    </router-link>
                    <router-link tag="div" class="index-tab-item" to="/index/notice">
                        <span class="tab-icon gonggao"></span>
                        <p class="tab-link">新闻资讯</p>
                    </router-link>
                    <router-link tag="div" class="index-tab-item" to="/index/paihang">
                        <span class="tab-icon paihang"></span>
                        <p class="tab-link">游戏排行</p>
                    </router-link>
                    <router-link tag="div" class="index-tab-item" to="/index/kaifu">
                        <span class="tab-icon kaifu"></span>
                        <p class="tab-link">近期开服</p>
                    </router-link>
                </div>
                <game-list :apps=item v-for="item in apps" :key=item.id></game-list>
            </div>
        </scroll>
        <mask-common :showMask="showMask" :maskInfo="maskInfo" @mask="hideMask" :update='update'></mask-common>
