      ...
      >
        <div v-if="downloadTasks.length || complatedTasks.length">
          ...
          >
            ...
            <div class="icon"><img
                v-lazy="taskInfo[ctask.url]&&taskInfo[ctask.url].icon"
                alt=""
              >
              <span
                class="delete"
            ...
            <div class="content">
              <h5>{{taskInfo[ctask.url]&&taskInfo[ctask.url].name?taskInfo[ctask.url].name:''}}</h5>
              <div class="precent">
                <div class="progress">
                  <div class="size">{{taskInfo[ctask.url]&&taskInfo[ctask.url].size?taskInfo[ctask.url].size:''}}</div>
          ...
          >
            <div class="icon"><img
                v-lazy="taskInfo[task.url] && taskInfo[task.url].icon"
                alt=""
              >
              <span
                class="delete"
            ...
            <div class="content">
              <h5>{{taskInfo[task.url]&&taskInfo[task.url].name?taskInfo[task.url].name:''}}</h5>
              <div class="precent">
                <div class="progress">
                  <div class="size">{{taskInfo[task.url]&&taskInfo[task.url].size?taskInfo[task.url].size:''}}</div>
    ...
    enumeratecTasks() {
      ...
      plus.downloader.enumerate((tasks) => {
        ...
        for (let j = 0; j < this.complatedTasks.length; j++) {
          ...
          console.log('包名', this.taskInfo[task.url] && this.taskInfo[task.url].pk_n ? this.taskInfo[task.url].pk_n : '包名是空的');
          if (plus.runtime.isApplicationExist({ pname: this.taskInfo[task.url].pk_n })) {
            this.$set(task, 'isInstall', true);
          } else {
            this.$set(task, 'isInstall', false);
          }
        }
        console.log('设置完成后的任务');
        console.log(this.complatedTasks);
      }, 4);
    },
    // 获取所有未完成任务，并且添加事件监听
